...
{
    public float scale_factor = 0.05f;
    public float MAXSCALE = 6.0f, MIN_SCALE = 1.0f; // zoom-in and zoom-out limits
    public float smoothDrag = 0.04f; // smoothness in drag
    ...
    void Start()
    {
        ...
        parentObject = this.gameObject;// new GameObject("ParentObject");
    ...
    void Update()
    {
        ...
        if (isMousePressed && Input.touchCount == 1 && Input.GetTouch(0).phase == TouchPhase.Moved && (parentObject.transform.localScale.x > MIN_SCALE || parentObject.transform.localScale.y > MIN_SCALE))
        {
            ...
            Vector3 diff = touch.deltaPosition * smoothDrag;
